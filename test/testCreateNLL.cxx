...
namespace {
  ...
  std::optional<FitSummary> runFit(RooWorkspace& workspace,
    ...
    try {
      ...
      if (backend == "codegen" || backend == "codegen_no_grad") {
        RooFit::Experimental::writeCodegenDebugMacro(*nll, "codegen");
      }
    ...
    if (!nll) {
      if (error)
        *error = "combineCreateNLL returned null pointer";
      return std::nullopt;
    }
  ...
  void compareFits(const FitSummary& baseline,
    ...
    for (const auto& entry : baseline.parameterErrors) {
      ...
      EXPECT_TRUE(diff / scale <= relTolerance)
          << "Parameter '" << name << "' error mismatch: combine=" << err1 << ", " << label << "=" << err2;
    }
  }
...
protected:
  ...
  void TearDown() override { runtimedef::set("MINIMIZER_no_analytic", oldAnalyticFlag_); }
  ...
  void runComparison() {
    ...
    if (cfg.backend == BackendComparison::Cpu) {
      ...
      // we don't use the analytic gradient yet. For now we just want to
...
TEST_P(CreateNLLTest, BackendConsistency) { runComparison(); }
...
// Alternative evaluation backends are only supported from ROOT 6.30.
    ...
    CreateNLLTest,
    ::testing::Values(
        //TestConfig{BackendComparison::Cpu, true, false},

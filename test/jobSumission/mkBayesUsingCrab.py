...
parser.add_option(
    ...
    default=100,
    type="int",
    help="Number of chains per worskpace among all job",
)
parser.add_option(
    "-m",
    "--mass",
    ...
    default="120",
    type="string",
    help="Mass, or comma-separated list of masses if using multiple workspaces",
)
parser.add_option(
    "-O",
    "--options",
...
if len(masses) == 1 and len(workspaces) != 1:
    masses = [masses[0] for w in workspaces]
elif len(workspaces) != len(masses):
    raise RuntimeError("You must specify a number of masses equal to the number of workspaces, or just one mass for all of them")

...
parser.add_option(
    "",
    "--parameterRanges",
    dest="parameterRanges",
    nargs=4,
    type="float",
    help="physics model parameter ranges (should be x1,x2,y1,y2) used to induce boundaries for physical regions. Default is range of POI points",
)
parser.add_option(
    "-P",
    "--POI",
...
for i, x in enumerate(points):
    ...
    ranges = "--setPhysicsModelParameterRanges %s=%g,%g:%s=%g,%g" % (
        options.POI[0],
        xrngemin,
        xrngemax,
        options.POI[1],
        yrngemin,
        yrngemax,
    )  # Needed for naming of HypoTestResults and physics boundaries
    toys = "$n"

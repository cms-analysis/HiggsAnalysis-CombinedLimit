...
void addLepPoint(LepBand &b, double x, TString file, double r=1.0) {
    ...
    double pmuObs = res->CLsplusb(), pmuObsErr = res->CLsplusbError();
    ...
    res->SetTestStatisticData(median+1e-6);    double clsMid = res->CLs(), pmuMid = res->CLsplusb();
    res->SetTestStatisticData(summer68+1e-6);  double cls68L = res->CLs(), pmu68L = res->CLsplusb();
    res->SetTestStatisticData(winter68+1e-6);  double cls68H = res->CLs(), pmu68H = res->CLsplusb();
    res->SetTestStatisticData(summer95+1e-6);  double cls95L = res->CLs(), pmu95L = res->CLsplusb();
    res->SetTestStatisticData(winter95+1e-6);  double cls95H = res->CLs(), pmu95H = res->CLsplusb();
    /*
    std::cout << "Expected CLs: median " << clsMid 
                                << ", 68% band [" << cls68L << ", " << cls68H << "]" 
                                << ", 95% band [" << cls95L << ", " << cls95H << "]" << std::endl;    
    std::cout << "Expected Pmu: median " << pmuMid 
                                << ", 68% band [" << pmu68L << ", " << pmu68H << "]" 
                                << ", 95% band [" << pmu95L << ", " << pmu95H << "]" << std::endl;    
    */

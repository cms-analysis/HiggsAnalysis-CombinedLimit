...
parser.add_option(
    ...
    type="string",
    default="CMS_channel",
    help="Category to split channels along",
)
parser.add_option(
    "-b",
    "--binned",
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Input data is a counting experiment",
)
parser.add_option(
    "-S",
    "--manual-split",
    ...
    default=False,
    action="store_true",
    help="Split datasets manually (necessary for binned-unbinned mixes)",
)
parser.add_option(
    "-v",
    "--verbose",
...
parser.add_option(
    ...
    dest="outn",
    type="string",
    default="{channel}_bData_{number}",
    help="Naming convention for output",
)
parser.add_option(
    "-o",
    "--out",
...
parser.add_option(
    ...
    default=0,
    type="int",
    help="First toy to include in the output file (note: numbers start from 1)",
)
parser.add_option(
    "--last",
    dest="last",
...
for fname in args:
    ...
    while True:
        ...
        if iToy < options.first:
            continue
        if iToy > options.last:
            break
        if options.verbose > 1:
            dataset.get().Print("V")
        obs = ROOT.RooArgSet(dataset.get())
        datasets = []
        if options.counting:
            ...
            for i in range(obsin.getSize()):
                ...
                data = ROOT.RooDataSet(
                    name,
                    name,
                    ROOT.RooArgSet(outw.var("CMS_fakeObs"), outw.var("_weight_")),
                    "_weight_",
                )
            ...
            obs.remove(cat)
            if options.verbose > 1:
                print(" observables in reduced dataset: ")
                obs.Print("V")
            datasets = []

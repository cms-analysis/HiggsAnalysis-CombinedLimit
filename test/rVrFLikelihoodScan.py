...
parser.add_option(
    ...
    "--physics-model",
    dest="physModel",
    default="HiggsAnalysis.CombinedLimit.PhysicsModel:strictSMLikeHiggs",
    type="string",
...
parser.add_option(
    ...
    "--out",
    dest="out",
    default="rVrFLikelihoodScan.root",
    type="string",
...
parser.add_option(
    ...
    default=None,
    type="string",
    help="decays to include (comma separated, if unspecified use all)",
)
parser.add_option(
    "--algo",
    dest="algo",
    default="grid",
    type="string",
    help="algorithm (only grid supported for now)",
)
parser.add_option(
    "-p",
    "--poi",
...
parser.add_option(
    ...
    type="string",
    action="append",
    help="POI values to set (poi=value,poi2=value2,...)",
)
parser.add_option(
    ...
    type="string",
    action="append",
    help="POI values to set (poi=min,max:poi2=min2,max2:...)",
)
parser.add_option(
    "--floatOtherPOI",
    dest="floatOtherPOI",
...
for decay, (prodv, prodf) in decays:
    if options.decays and decay not in options.decays:
        continue
    ## get scaling factors
...
if options.algo == "grid":
    ...
    if options.points == 0:
        options.points = int(pow(200, len(poi)))
    ...
    if len(poi) == 1:
        ...
        for i in range(options.points):
            ...
            poiList.at(0).setVal(x)
            poi[0][0] = x
            if mustMinim and likelihood.getVal() < 999:
                constrMinimizer.minimize("Minuit", "minimize")

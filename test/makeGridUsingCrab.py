...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Run on LSF instead of GRID (can be changed in .cfg file)",
)
parser.add_option(
    "--condor",
    dest="condor",
    default=False,
    action="store_true",
    help="Run on condor_g instead of GRID (can be changed in .cfg file)",
)
parser.add_option(
    "--glidein",
    dest="glide",
    default=False,
    action="store_true",
    help="Use glide-in scheduler instead of glite",
)
parser.add_option(
    "--server",
    dest="server",
...
parser.add_option(
    ...
    default="8nh",
    type="string",
    help="LSF queue to use (can be changed in .cfg file)",
)
parser.add_option(
    "-O",
    "--options",
    ...
    default="--freq",
    type="string",
    help="options to use for combine",
)
parser.add_option(
    "-n",
    "--points",
    ...
    default=10,
    type="int",
    help="Points to choose in the range (note: both endpoints are included)",
)
parser.add_option(
    "-T",
    "--toysH",
    ...
    default=500,
    type="int",
    help="Toys per point per iteration",
)
parser.add_option(
    "-t",
    "--toys",
    ...
    default=50,
    type="int",
    help="Total number of iterations per point among all jobs (can be changed in .cfg file)",
)
parser.add_option(
    "-j",
    "--jobs",
    ...
    default=10,
    type="int",
    help="Total number of jobs (can be changed in .cfg file)",
)
parser.add_option(
    "-I",
    "--interleave",
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Use random seeds for the jobs",
)
parser.add_option(
    "-P",
    "--priority",
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Run more toys at low edge of the band, to get better low range",
)
parser.add_option(
    "-S",
    "--signif",
    ...
    default=False,
    action="store_true",
    help="Compute significance. You should set min = 1, max = 1",
)
parser.add_option(
    "-u",
    "--uidir",
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Run MaxLikelihoodFit with toys for diagnostic studies",
)
...
if (min != max) and options.diagnosticRun:
    sys.exit("Cannot run diagnostics with > 1 expected signal (set min=max=prefit signal")
...
if options.diagnosticRun:
    script.write("hadd mlfit%s.root mlfit*.root\n" % options.out)
...
""".format(
        wsp=workspace,
        out="mlfit%s.root,%s.root" % (options.out, options.out) if options.diagnosticRun else options.out + ".root",
        scriptout=options.out,
        sched=sched,
        srv=(1 if options.server else 0),
        queue=options.queue,
        jobs=options.j,
        total=options.t,

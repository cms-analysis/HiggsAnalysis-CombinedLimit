...
suite += [
    (
        ...
        MultiDatacardTest(
            "Counting_Lower",
            datacardGlob("simple-counting/counting-B5p5-Obs[12][10]-Syst30U.txt"),
            M,
            "--lowerLimit",
        ),
    )
...
suite += [
    (
        M,
        "*",
        MultiDatacardTest("Counting_ZeroB", datacardGlob("simple-counting/counting-B0-*.txt"), M, ""),
    )
]
suite += [
    (
        ...
        "*",
        MultiDatacardTest(
            "Counting_ZeroB_Lower",
            datacardGlob("simple-counting/counting-B0-*.txt"),
            M,
            "--lowerLimit",
        ),
    )
...
for d in datacardGlob("hww4ch-1fb-S*mH*.txt"):
    ...
    suite += [
        (
            M,
            "full",
            SingleDatacardTest("HWW_Sig_Lower_" + n, d, M, "-H ProfileLikelihood --lowerLimit"),
        )
    ]

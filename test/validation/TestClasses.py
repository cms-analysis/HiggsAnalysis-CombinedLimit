...
def _readRootFile(fname):
    ...
    if not f:
        return {"status": "aborted", "comment": "rootfile is not readable by ROOT"}
    t = f.Get("limit")
    if not t:
        f.Close()
    ...
    for c in ["limit", "limitErr", "t_real"]:
        result[c] = t.GetMaximum(c)
    f.Close()
    if abs(result["limit"]) > 1e6:
        return {
            "status": "aborted",
            "comment": "rootfile %s contains bogus data" % fname,
        }
    return result
...
class SingleDatacardTest(Test):
    ...
    def createScript(self, dir, file):
        ...
        file.write(
            "combine %s -n %s -M %s %s -m %s 2>&1 | cat >> %s.log\n"
            % (
                datacard_full,
                self._name,
                self._method,
    ...
    def readOutput(self, dir):
        fname = "%s/higgsCombine%s.%s.mH%s.root" % (
            dir,
            self._name,
            self._method,
            self._mass,
        ...
        if "Median" in self._name and "Hybrid" in self._method:
            fname = "%s/higgsCombine%s.%s.mH%s.quant0.500.root" % (
                dir,
                self._name,
                self._method,
                self._mass,
        ...
        return {
            "status": "done",
            "comment": "",
            "results": {os.path.basename(self._datacard): out},
        }
...
class MultiDatacardTest(Test):
    ...
    def createScript(self, dir, file):
        for dc, mass in self._datacards:
            ...
            file.write(
                "combine %s -n %s -M %s %s -m %s 2>&1 | cat >> %s.%s.log\n"
                % (
                    datacard_full,
                    self._name,
                    self._method,
...
class WorkspaceWithExpectedTest(MultiDatacardTest):
    ...
    def readOutput(self, dir):
        if "MultiDimFit" in self._method:
            out = _readRootFileWithPOI(
                "%s/higgsCombine%s.%s.mH%s.root" % (dir, self._name, self._method, self._mass),
                self._poi,
            )
        else:
            out = _readRootFileWithExpected("%s/higgsCombine%s.%s.mH%s.root" % (dir, self._name, self._method, self._mass))
...
def datacardGlob(pattern):
    ...
    if ret == []:
        raise RuntimeError("Pattern '%s' does not expand to any datacard in data/benchmarks." % pattern)
    return ret

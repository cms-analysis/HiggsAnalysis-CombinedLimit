...
class TestSuite:
    def __init__(self, dir, options, allTests):
        ...
        for m, l, test in allTests:
            if m != options.method and options.method != "*":
                continue
            if l != "*" and options.suite != l:
                continue
            if options.select and not re.search(options.select, test.name()):
                continue
            if options.exclude and re.search(options.exclude, test.name()):
                continue
            if test.name() in dups:
                raise RuntimeError("Duplicate test %s" % test.name())
            if options.nofork:
                test.forceSingleCPU()
    ...
    def runBatch(self, queue="8nh", command="bsub -q %(queue)s %(script)s"):
        for t in self._tests:
            cmd = command % {
                "queue": queue,
                "script": os.getcwd() + "/" + t.scriptName(self._dir),
            }
    ...
    def printIt(self, format, reference=None):
        ...
        if format == "twiki":
            ...
            RuntimeError, "Unknown format %s" % format

    def _selTests(self, method="*", length="full"):
        jobs = []
        for m, l, t in self._tests:
            if m != method and method != "*":
                continue
            if l == "full" and length == "short":
                continue
            jobs.append(t)
        return jobs
    ...
    def _createDir(self, clear=False):
        ...
        if not os.access(self._dir, os.W_OK):
            os.mkdir(self._dir)
        if not os.access(self._dir, os.W_OK):
            RuntimeError, "Could not create dir %d" % self._dir

    def _collate(self, report, reference):
        for tn, tv in report.items():
            if tn not in reference:
                continue
            if "results" not in tv:
                continue
            if tv["status"] == "aborted":
                continue
            tv_ref = reference[tn]
            tv["ref_comment"] = tv_ref["comment"]
            if "results" not in tv_ref:
                continue
            for name, res in tv["results"].items():
                if res["status"] != "done":
                    continue
                if name not in tv_ref["results"]:
                    continue
            ...
            if len(tv["results"]) == 1:
                ...
                if errors > 0:
                    tv["status"] = "error"
                    tv["comment"] = "%d errors, %d warnings" % (errors, warnings)
                    report["has_ref"] = True
                elif warnings > 0:
                    tv["status"] = "warning"
                    tv["comment"] = "%d warnings" % warnings
                    report["has_ref"] = True
                elif aborts > 0:
                    tv["status"] = "mixed"
                elif "has_ref" in tv:

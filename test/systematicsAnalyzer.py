...
parser.add_option(
    ...
    default=None,
    type="string",
    help="Higgs process to use. Will also be written in the Workspace as RooRealVar 'MH'.",
)
parser.add_option(
    "-D",
    "--dataset",
...
parser.add_option(
    ...
    action="append",
    type="string",
    help="Selection of nuisance parameters (regexp, can be used multiple times)",
)
parser.add_option(
    "-a",
    "--all",
    ...
    default=False,
    action="store_true",
    help="Report all nuisances (default is only lnN)",
)
parser.add_option(
    "",
    "--t2w",
    ...
    default=False,
    action="store_true",
    help="Run text2workspace -- only effects datacards with param lines",
)
parser.add_option(
    "",
    "--noshape",
...
for lsyst, nofloat, pdf, pdfargs, errline in DC.systs:
    ...
    if not options.all and pdf != "lnN":
        continue
    if not options.t2w and "param" in pdf:
        continue
    if "param" in pdf:
    ...
    vals = []
    if "param" in pdf and not MODELBUILT:
        continue
    for b in DC.bins:
        numKeysFound = 0
        channels.append(b)
        for p in six.iterkeys(DC.exp[b]):
            ...
            if pdf == "gmN":
                ...
                if not MB.out.function(formula):
                    formula = "n_exp_bin%s_proc_%s" % (b, p)
                if not MB.out.function(formula):
                    sys.exit("No formula %s" % formula)
                if not (MB.out.function(formula).getParameters(ROOT.RooArgSet())).contains(MB.out.var(lsyst)):
                    continue
                ...
                errlines[lsyst][b][p] = "%.3f/%.3f (param) " % (
                    lowNorm / centralNorm,
                    highNorm / centralNorm,
                )
                ...
                systShapeName = lsyst
                # vals = []
                if (lsyst, b, p) in list(DC.systematicsShapeMap.keys()):
                    systShapeName = DC.systematicsShapeMap[(lsyst, b, p)]

                objU, objD, objC = (
                    MB.getShape(b, p, systShapeName + "Up"),
                    MB.getShape(b, p, systShapeName + "Down"),
                    MB.getShape(b, p),
                )

                if objC.InheritsFrom("TH1"):
                    valU, valD, valC = objU.Integral(), objD.Integral(), objC.Integral()
                elif objC.InheritsFrom("RooDataHist"):
                    valU, valD, valC = (
                        objU.sumEntries(),
                        objD.sumEntries(),
                        objC.sumEntries(),
                ...
                if valC != 0:
                    errlines[lsyst][b][p] = "%.3f/%.3f (shape)" % (
                        valD / valC,
                        valU / valC,
                    )
    ...
    if lsyst in list(report.keys()):
        ...
        report[lsyst]["effect"] = [
            "%5.3f" % (min(float(report[lsyst]["effect"][0]), minEffect)),
            "%5.3f" % (max(float(report[lsyst]["effect"][1]), maxEffect)),
        ]
        if types not in report[lsyst]["types"]:
            report[lsyst]["types"] += "," + types
    else:
        report[lsyst] = {
            "channels": channelsShort,
            "bins": channels,
            "processes": sorted(processes.keys()),
            "effect": ["%5.3f" % minEffect, "%5.3f" % maxEffect],
            "types": types,
        }
...
"""
    ...
    for nuis in names:
        ...
        print(
            "<td>%s" % ", ".join(["%s(%d)" % (k, v) for (k, v) in sorted(val["channels"])]),
            end=" ",
        )

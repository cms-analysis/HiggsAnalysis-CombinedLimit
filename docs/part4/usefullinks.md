...
* _Why does my fit not converge?_ 
    ...
    * We have  seen often that fits in combine using `RooCBShape` as a parametric function will fail. This is related to an optimisation that fails. You can try to fix the problem as described in this issue: [https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/issues/347](https://github.com/cms-analysis/HiggsAnalysis-CombinedLimit/issues/347) (i.e add the option `--X-rtd ADDNLL_CBNLL=0`).

...
In our single mass point analysis one can check indeed the behaviour illustrated in the limit plot above by looking at the $50$ % quantile on the signal strength $r$ after running the limit with and without signal injection in the control regions. Moreover, the effect of non-closure can be seen looking at the observed limit on $r$.

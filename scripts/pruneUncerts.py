...
def main():
    ...
    for idx in range(len(fit_results)):
        fit_results[idx] = fit_results[idx].rstrip(",")
    fit_results = remove_empty_strings(fit_results)
    ## turn options.blacklist into a list
    blacklist = options.blacklist.split(" ")
    for idx in range(len(blacklist)):
        blacklist[idx] = blacklist[idx].rstrip(",")
    ...
    for idx in range(len(whitelist)):
        whitelist[idx] = whitelist[idx].rstrip(",")

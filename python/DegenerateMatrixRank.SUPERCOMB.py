...
class AllMuiLambdaHiggs(SMLikeHiggsModel):
	...
	for decay in self.decays:
            ...
            self.modelBuilder.factory_("expr::lambdawmu_%s(\"@0*@1\",lambdaw,mu_%s)" % (decay,decay))
            self.modelBuilder.factory_("expr::lambdazmu_%s(\"@0*@1\",lambdaz,mu_%s)" % (decay,decay))
	...
	if decay in [ "hgluglu", "hcc", "hss", "huu", "hdd" ]:
        ...
        if production == "ggH" or production == "bbH":
                ...
                return 'lambdazmu_'+decay
        elif production == "WH":
                return 'lambdawmu_'+decay
...
class AllMuiLambdasHiggs(SMLikeHiggsModel):
    ...
    def doParametersOfInterest(self):
	    ...
	    self.modelBuilder.doVar("lambdaz_%s[1,0,10]" % decay)
            ...
            self.modelBuilder.factory_("expr::lambdaz_%smu_%s(\"@0*@1\",lambdaz_%s,mu_%s)" % (decay,decay,decay,decay))
            self.modelBuilder.factory_("expr::lambdaw_%smu_%s(\"@0*@1\",lambdaw_%s,mu_%s)" % (decay,decay,decay,decay))
	...
	if production == "ggH" or production == "bbH":
                ...
                return 'lambdaz_'+decay+'mu_'+decay
        elif production == "WH":
                return 'lambdaw_'+decay+'mu_'+decay
        elif production == "ttH" or production == "tH":

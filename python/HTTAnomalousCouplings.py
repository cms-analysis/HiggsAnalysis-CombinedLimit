...
xsecs = {
    ...
    "yield_Powheg_ggH": 5.127128e00,
    "yield_SM_ggH": 7.937751e01,
    "yield_BSM_ggH": 8.952848e01,
...
class Anomalous_Interference_JHU_rw(PhysicsModelBase_NiceSubclasses):
    ...
    def getPOIList(self):
        ...
        self.modelBuilder.factory_(
            'expr::smCoupling_VBF("@0*@3*@1**2 - {aisign}@0*@1*@2*@3*sqrt({sigmai_VBF}/{sigma1_VBF})", {muV},a1,ai,muTT)'.format(**myxsecs)
        )
        ...
        self.modelBuilder.factory_(
            'expr::bsmCoupling_VBF("@0*@3*@1**2*{sigmai_VBF}/{sigma1_VBF} - {aisign}@0*@1*@2*@3*sqrt({sigmai_VBF}/{sigma1_VBF})", {muV},ai,a1,muTT)'.format(
                **myxsecs
            )
        )
        self.modelBuilder.factory_(
            'expr::bsmCoupling_ZH("@0*@3*@1**2*{sigmai_ZH}/{sigma1_ZH} - {aisign}@0*@1*@2*@3*sqrt({sigmai_ZH}/{sigma1_ZH})", {muV},ai,a1,muTT)'.format(
                **myxsecs
            )
        )
        self.modelBuilder.factory_(
            'expr::bsmCoupling_WH("@0*@3*@1**2*{sigmai_WH}/{sigma1_WH} - {aisign}@0*@1*@2*@3*sqrt({sigmai_WH}/{sigma1_WH})", {muV},ai,a1,muTT)'.format(
                **myxsecs
            )
        )

        self.modelBuilder.factory_(
            'expr::intCoupling_VBF("{aisign}@0*@1*@2*@3*sqrt({sigmai_VBF}/{sigma1_VBF})*{sigmaint_VBF}/{sigma1_VBF}", {muV},a1,ai,muTT)'.format(**myxsecs)
        )
        self.modelBuilder.factory_(
            'expr::intCoupling_ZH("{aisign}@0*@1*@2*@3*sqrt({sigmai_ZH}/{sigma1_ZH})*{sigmaint_ZH}/{sigma1_ZH}", {muV},a1,ai,muTT)'.format(**myxsecs)
        )
        self.modelBuilder.factory_(
            'expr::intCoupling_WH("{aisign}@0*@1*@2*@3*sqrt({sigmai_WH}/{sigma1_WH})*{sigmaint_WH}/{sigma1_WH}", {muV},a1,ai,muTT)'.format(**myxsecs)
        )
    ...
    def getYieldScale(self, bin, process):
        ...
        if process in [
            ...
            if not self.dofa3gg:
                raise ValueError("You have GGH2Jets_pseudoscalar_M, but did not set dofa3gg=true")
        ...
        for anomalouscoupling, pureBSM in six.iteritems(pureBSMs):
            ...
            if process in [
                "reweighted_qqH_htt_" + pureBSM,
            ]:
                return "bsmCoupling_VBF"
            if process in [
                "reweighted_WH_htt_" + pureBSM,
            ]:
                return "bsmCoupling_WH"
            if process in [
                "reweighted_ZH_htt_" + pureBSM,
            ]:

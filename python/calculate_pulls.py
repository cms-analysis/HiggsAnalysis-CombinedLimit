...
def allowed_methods():
    return [
        ...
        "relDiffAsymErrs",
        "diffPullAsym",
    ]  # only allow asymm versions for now (just symmetrise errors for symmetric version)
...
def compat(x, x0, sx, sx0):
    ...
    sxo = 1.0 / ((1.0 / (sx * sx)) - (1.0 / sx0 * sx0))
    xo = (x - (1.0 / (sx0 * sx0)) * x0) * sxo
...
def compatAsym(x, x0, sxu, sxu0, sxd, sxd0):
    ...
    if x > x0:
        ret = compat(x, x0, sxd, sxd0)
    else:
        ret = compat(x, x0, sxu, sxu0)
    ret.append(0)
    return ret
...
def diffPullAsym(x, x0, sxu, sxu0, sxd, sxd0):
    # as defined in http://physics.rockefeller.edu/luc/technical_reports/cdf5776_pulls.pdf
    if x > x0:
        ret = diffPull(x, x0, sxd, sxd0)
    else:
        ret = diffPull(x, x0, sxu, sxu0)
    ret.append(ret[-1])
    return ret
...
def unconstPullAsym(x, x0, sxu, sxu0, sxd, sxd0):
    if x < x0:
        ret = unconstPull(x, x0, sxu)
    else:
        ret = unconstPull(x, x0, sxd)
    ret.append(0)
    return ret
...
def returnPull(method, x, x0, sx, sx0):
    if method == "compat":
        return compat(x, x0, sx, sx0)
    elif method == "diffPull":
...
def returnPullAsym(method, x, x0, sxu, sxu0, sxd, sxd0):
    if method == "unconstPullAsym":
        return unconstPullAsym(x, x0, sxu, sxu0, sxd, sxd0)
    elif method == "relDiffAsymErrs":
        return relDiffAsymErrs(x, x0, sxu, sxu0, sxd, sxd0)
    elif method == "compatAsym":
        return compatAsym(x, x0, sxu, sxu0, sxd, sxd0)
...
def returnTitle(method):
    if method in ["unconstPullAsym", "unconstPull"]:
        return "(#theta-#theta_{I})/#sigma"
    elif method in ["relDiffAsymErrs"]:
        return "(#theta-#theta_{I})/#sigma_{I}"
    elif method in ["compat", "compatAsym"]:
        return "#chi^{2} compat."

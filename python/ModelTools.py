...
class ModelBuilder(ModelBuilderBase):
    ...
    def doFillNuisPdfsAndSets(self):
        if self.options.bin:
            ...
            setNuisPdf = list(dict.fromkeys((setNuisPdf))) #set(setNuisPdf)
            for n in setNuisPdf:
            ...
            # avoid duplicating  _Pdf in list
            setNuisPdf = list(dict.fromkeys(keywords([self.getSafeNormName(n) for (n, nf, p, a, e) in self.DC.systs]))) #set([self.getSafeNormName(n) for (n, nf, p, a, e) in self.DC.systs])

...
class STXStoEFTBaseModel(SMLikeHiggsModel):
    ...
    def __init__(
        ...
        self.freezeOtherParameters = (
            freezeOtherParameters  # Option to freeze majority of parameters in model. Leaving those used in LHCHXSWG-INT-2017-001 fit to float
        )
        ...
        if self.freezeOtherParameters:
            self.parametersOfInterest = [
                ...
                "cd",
                "cl",
            ]  # note cWW+cB is frozen, but required to define cWW and cB
            self.distinctParametersOfInterest = set()
            for p in self.parametersOfInterest:
                if "Plus" in p:
                    self.distinctParametersOfInterest = self.distinctParametersOfInterest | set(p.split("Plus"))
                elif "Minus" in p:
                    self.distinctParametersOfInterest = self.distinctParametersOfInterest | set(p.split("Minus"))
                else:
                    self.distinctParametersOfInterest.add(p)
    ...
    def setPhysicsOptionsBase(self, physOptions):
        for po in physOptions:
            ...
            if po.startswith("STXSU="):
                self.doSTXSU = po.replace("STXSU=", "") in [
                    "yes",
                    "1",
                    "Yes",
                    "True",
            ...
            if po.startswith("useExtendedVBFScheme="):
                self.useExtendedVBFScheme = po.replace("useExtendedVBFScheme=", "") in [
                    "yes",
                    "1",
                    "Yes",
                    "True",
            ...
            if po.startswith("useLHCHXSWGStage1="):
                self.useLHCHXSWGNumbers = po.replace("useLHCHXSWGStage1=", "") in [
                    "yes",
                    "1",
                    "Yes",
                    "True",
            ...
            if po.startswith("linearOnly="):
                self.linearOnly = po.replace("linearOnly=", "") in [
                    "yes",
                    "1",
                    "Yes",
                    "True",
        ...
        if self.doSTXSU:
            print(" --> [WARNING]: theory uncertainties in STXS bins are currently incorrect. Need to update: data/lhc-hxswg/eft/HEL/*_binuncertainties.txt")
        if self.freezeOtherParameters:
            print(" --> [STXStoEFT] Freezing all but [cG,cu,cd,cHW,cHB,cA,cWWMinuscB,cl] (distinct: %s) to 0" % (self.distinctParametersOfInterest))
        else:
            print(" --> [STXStoEFT] Allowing all HEL parameters to float")
        if len(self.fixProcesses) > 0:
            print(" --> [STXStoEFT] Fixing following processes to SM: %s" % self.fixProcesses)
        if self.useExtendedVBFScheme:
            print(" --> [STXStoEFT] Use extended VBF scheme (different scaling for pure + V(qq)H)")
        if self.useLHCHXSWGNumbers:
            print(" --> [STXStoEFT] Using LHCHXSWG numbers for stage 1 scaling functions")
        if self.linearOnly:
            print(" --> [STXStoEFT] Only linear terms (Ai)")
        ...
        for line in lines[skipRows:]:
            ...
            if self.freezeOtherParameters:
                if line.split()[0].split("_")[0] not in self.parametersOfInterest:
                    skip_poi = True
            if not skip_poi:
                self.pois["%s" % line.split()[0]] = "[{},{},{}]".format(
                    line.split()[1],
                    line.split()[2],
                    line.split()[3],
                )
                ...
                if len(poi_scale) == 1:
                    self.poi_scaling[poi_scale[0]] = "1.*%s" % line.split()[0]
                else:
                    if poi_scale[1] == "x05":
                        self.poi_scaling[poi_scale[0]] = "0.00001*%s" % line.split()[0]
                    elif poi_scale[1] == "x04":
                        self.poi_scaling[poi_scale[0]] = "0.0001*%s" % line.split()[0]
                    elif poi_scale[1] == "x03":
                        self.poi_scaling[poi_scale[0]] = "0.001*%s" % line.split()[0]
                    elif poi_scale[1] == "x02":
                        self.poi_scaling[poi_scale[0]] = "0.01*%s" % line.split()[0]
                    elif poi_scale[1] == "x01":
                        self.poi_scaling[poi_scale[0]] = "0.1*%s" % line.split()[0]
                    else:
                        raise ValueError("[ERROR] Parameter of interest scaling not in allowed range [0.1-0.0001]")
                # if poi: cG, cA, tcG, tcA add factor of 16pi2
                if poi_scale[0] in ["cG", "tcG", "cA", "tcA"]:
                    self.poi_scaling[poi_scale[0]] = "157.914*%s" % (self.poi_scaling[poi_scale[0]])
        file_in.close()
    ...
    def makeScalingFunction(self, what, STXSstage=""):
        ...
        elif what in self.DecayScalingFunctions:
            formula = self.DecayScalingFunctions[what]
        else:
            if STXSstage != "":
                raise ValueError(f"[ERROR] Scaling function for {what} does not exist for STXS Stage {STXSstage}")
            else:
        ...
        for term in formula[1:]:
            ...
            if self.freezeOtherParameters:
                for c in constituents[1:]:  # Ignore pre-factor
                    if c not in self.distinctParametersOfInterest:
                        skipTerm = True
            ...
            if not skipTerm:
                ...
                for c in constituents:
                    if c in self.poi_scaling:
                        scaled_constituents.extend(self.poi_scaling[c].split("*"))
                    else:
                        scaled_constituents.append(c)
                # define string for term: RooProduct (factory)
                termStr = f"prod::term_{what}_{_termIdx:g}("
                for sc in scaled_constituents:
                    termStr += "%s," % sc
                termStr = termStr[:-1] + ")"
                # add term to model and dictionary
                self.modelBuilder.factory_(termStr)
                formulaTerms.append(f"term_{what}_{_termIdx:g}")
        ...
        for _termIdx in range(len(formulaTerms)):
            if _termIdx % 15 == 0:
                _sumIdx += 1
            # Add terms to sum
            if (what in self.DecayScalingFunctions) & (what != "tot"):
                sumString = f"scaling_partial_{what}_{_sumIdx}"
            else:
                sumString = f"scaling_{what}_{_sumIdx}"
            if sumString in sumTerms:
                sumTerms[sumString] += "%s," % formulaTerms[_termIdx]
            else:
                sumTerms[sumString] = "%s," % formulaTerms[_termIdx]
        # Add sizeable sums as RooAdditions
        for key, value in sumTerms.items():
            self.modelBuilder.factory_(f"sum::{key}({value[:-1]})")

        # Define string for total: 1 + sizeable sums
        if (what in self.DecayScalingFunctions) & (what != "tot"):
            totalStr = "sum::scaling_partial_%s(1," % what
        else:
    ...
    def makeBRScalingFunction(self, what):
        self.modelBuilder.factory_(f'expr::scaling_BR_{what}("@0/@1", scaling_partial_{what}, scaling_tot)')

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Function extracting the STXS bin uncertainties
    def makeSTXSBinUncertainties(self, STXSstage="1"):
        if STXSstage == "all":
            stage_list = ["0", "1", "1_1", "fixedproc"]
        else:
            stage_list = [STXSstage, "fixedproc"]
        storedUncertainties = []
        for s in stage_list:
            if s == "fixedproc":
                key = s
            ...
            for line in open(os.path.join(self.SMH.datadir, "eft/HEL/%s_binuncertainties.txt" % key)):
                if stxsUncertaintiesKeys == []:
                    ...
                    fields = line.split()
                    stxsUncertainties[fields[0]] = {k: 0.01 * float(v) for (k, v) in zip(stxsUncertaintiesKeys, fields[1:])}
            ...
            for proc in self.PROCESSES[key]:
                ...
                if proc not in stxsUncertainties:
                    self.modelBuilder.doVar(f"{key}_UncertaintyScaling_{proc}[1]")
                    continue
                else:
                    pnorm = ROOT.ProcessNormalization(f"{key}_UncertaintyScaling_{proc}", "")
                    for _u in stxsUncertaintiesKeys:
...
class AllStagesToEFTModel(STXStoEFTBaseModel):
    def __init__(self):
        ...
        from HiggsAnalysis.CombinedLimit.STXS import (
            fixed_procs,
            stage0_procs,
            stage1_procs,
            stage1_1_procs,
        ...
        for s in ["0", "1", "1_1"]:
            if s == "0":
                self.PROCESSES["stage%s" % s] = [x for v in stage0_procs.values() for x in v]
            elif s == "1":
                self.PROCESSES["stage%s" % s] = [x for v in stage1_procs.values() for x in v]
            else:
                self.PROCESSES["stage%s" % s] = [x for v in stage1_1_procs.values() for x in v]
    ...
    def doParametersOfInterest(self):
        if self.floatMass:
            print(" --> [WARNING] Floating Higgs mass selected. STXStoEFT model assumes MH=125.0 GeV")
        self.doMH()
        ...
        for poi, poi_range in self.pois.items():
            self.modelBuilder.doVar(f"{poi}{poi_range}")
        # Remove cWW+cB from POI list if freezing other parameters
        if self.freezeOtherParameters:
            POIs = re.sub("cWWPluscB_x02,", "", POIs)
        ...
        if self.freezeOtherParameters:
            for poi in self.pois:
                if "cWWPluscB" in poi:
                    self.modelBuilder.out.var(poi).setConstant(True)
    ...
    def setup(self):
        ...
        if self.doSTXSU:
            self.makeSTXSBinUncertainties(STXSstage="all")
        ...
        for s in ["1", "0", "1_1"]:
            ...
            if s == "1" and self.useLHCHXSWGNumbers:
                self.textToSTXSScalingFunctions(
                    os.path.join(
                        self.SMH.datadir,
                        "eft/HEL/%s_xs_LHCHXSWG-INT-2017-001.txt" % stage,
                    )
                )
            else:
                if self.useExtendedVBFScheme:
                    self.textToSTXSScalingFunctions(os.path.join(self.SMH.datadir, "eft/HEL/%s_xs_extended_vbf.txt" % stage))
                else:
        ...
        for proc in self.PROCESSES["fixedproc"]:
            self.modelBuilder.factory_('expr::scaling_%s("@0",1.)' % proc)
        ...
        for dec in self.DECAYS:
            self.makeScalingFunction(dec)
        # Make BR scaling functions: partial width/total width
        for dec in self.DECAYS:
            if dec != "tot":
                self.makeBRScalingFunction(dec)
    ...
    def getHiggsSignalYieldScale(self, production, decay, energy):
        ...
        if self.modelBuilder.out.function(name) is None:
            ...
            if production in self.PROCESSES["stage1_1"]:
                ...
                key = "stage1"
            elif production in self.PROCESSES["stage0"]:
                key = "stage0"
            ...
            if (self.doSTXSU) & (self.doBRU):
                if key is None:
                    ...
                    THUscaler = f"uncertainty_scaling_{production}_{decay}"
                    self.modelBuilder.factory_(
                        'expr::uncertainty_scaling_%s_%s("@0*@1",%s_UncertaintyScaling_%s,HiggsDecayWidth_UncertaintyScaling_%s)'
                ...
                if key is None:
                    ...
                    self.modelBuilder.factory_(f'expr::uncertainty_scaling_{production}("@0",{key}_UncertaintyScaling_{production})')
            elif self.doBRU:
                THUscaler = "uncertainty_scaling_%s" % decay
                self.modelBuilder.factory_(f'expr::uncertainty_scaling_{decay}("@0",HiggsDecayWidth_UncertaintyScaling_{decay})')
            ...
            if (self.doSTXSU) | (self.doBRU):
                ...
                self.modelBuilder.factory_("prod::{}({})".format(name, ",".join([XSscal, BRscal])))
...
class StageXToEFTModel(STXStoEFTBaseModel):
    def __init__(self, stage):
        ...
        self.PROCESSES["stage%s" % self.stage] = [x for v in getattr(STXS, "stage%s_procs" % self.stage).values() for x in v]
    ...
    def setup(self):
        ...
        if self.doBRU:
            self.SMH.makePartialWidthUncertainties()
        if self.doSTXSU:
            self.makeSTXSBinUncertainties(STXSstage=self.stage)
        ...
        for proc in self.PROCESSES["stage%s" % self.stage]:
            self.makeScalingFunction(proc, STXSstage=self.stage)
    ...
    def getHiggsSignalYieldScale(self, production, decay, energy):
        ...
        if self.modelBuilder.out.function(name) is None:
            ...
            if production in self.PROCESSES["stage%s" % self.stage]:
                ...
                raise ValueError(f"[ERROR] Process {production} is not supported in STXStoEFT Model (stage {self.stage})")
            # Give production correct scaling
            XSscal = "scaling_%s" % production
...
def convert_to_STXS(_production, _decay):
    ...
    if _production in ["WH", "ZH", "ggZH"]:
        _production = "%s_lep" % _production

    # hzz: NIGHTMARE
    if _decay == "hzz":
        # VBFori: set to qqH
        if "VBFori" in _production:
            _production = "qqH"

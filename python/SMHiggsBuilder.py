...
class SMHiggsBuilder:
    ...
    def makeXS(self, process, energy="7TeV"):
        ...
        if process == "ggH":
            self.textToSpline("SM_XS_ggH_" + energy, os.path.join(self.xspath, energy + "-ggH.txt"))
        if process == "qqH":
            self.textToSpline("SM_XS_qqH_" + energy, os.path.join(self.xspath, energy + "-vbfH.txt"))
        if process == "ttH":
            self.textToSpline("SM_XS_ttH_" + energy, os.path.join(self.xspath, energy + "-ttH.txt"))
        if process == "WH":
            self.textToSpline("SM_XS_WH_" + energy, os.path.join(self.xspath, energy + "-WH.txt"))
        if process == "ZH":
            self.textToSpline("SM_XS_ZH_" + energy, os.path.join(self.xspath, energy + "-ZH.txt"))
        if process == "ggZH":
            self.textToSpline("SM_XS_ggZH_" + energy, os.path.join(self.xspath, energy + "-ggZH.txt"))
        if process == "bbH":
            self.textToSpline("SM_XS_bbH_" + energy, os.path.join(self.xspath, energy + "-bbH.txt"))
        if process == "tHq":
            self.textToSpline("SM_XS_tHq_" + energy, os.path.join(self.xspath, energy + "-tHq.txt"))
        if process == "tHW":
    ...
    def makeBR(self, decay):
        if decay == "hww":
            self.textToSpline("SM_BR_hww", os.path.join(self.brpath, "BR4.txt"), ycol=10)
        if decay == "hzz":
            self.textToSpline("SM_BR_hzz", os.path.join(self.brpath, "BR4.txt"), ycol=11)
        if decay == "hgg":
            self.textToSpline("SM_BR_hgg", os.path.join(self.brpath, "BR4.txt"), ycol=8)
        if decay == "hzg":
            self.textToSpline("SM_BR_hzg", os.path.join(self.brpath, "BR4.txt"), ycol=9)
        if decay == "hbb":
            self.textToSpline("SM_BR_hbb", os.path.join(self.brpath, "BR4.txt"), ycol=1)
        if decay == "htt":
            self.textToSpline("SM_BR_htt", os.path.join(self.brpath, "BR4.txt"), ycol=2)
        if decay == "hmm":
            self.textToSpline("SM_BR_hmm", os.path.join(self.brpath, "BR4.txt"), ycol=3)
        if decay == "hss":
            self.textToSpline("SM_BR_hss", os.path.join(self.brpath, "BR4.txt"), ycol=5)
        if decay == "hcc":
            self.textToSpline("SM_BR_hcc", os.path.join(self.brpath, "BR4.txt"), ycol=4)
        if decay == "hgluglu":
            self.textToSpline("SM_BR_hgluglu", os.path.join(self.brpath, "BR4.txt"), ycol=7)
        if decay == "htoptop":
            self.textToSpline("SM_BR_htoptop", os.path.join(self.brpath, "BR4.txt"), ycol=6)
 ...
 %(rooName)s\
            ...
            for sqrts in ("7TeV", "8TeV", "13TeV", "14TeV"):
                for qty, column in structure.items():
                    ...
                    self.textToSpline(
                        rooName,
                        os.path.join(self.coupPath, "ggH_%(sqrts)s.txt" % locals()),
                        ycol=column,
                    )
                scalingName = "Scaling_" + what + "_" + sqrts
                #                print 'Building '+scalingName
...
"((@0*@0)*@3  + (@1*@1)*@4 + (@0*@1)*@5 + (@0*@2)*@6 + (@1*@2)*@7 + (@2*@2)*@8)/@9 ",\
 ...
 %(prefix)sc_kt2_%(sqrts)s, %(prefix)sc_kb2_%(sqrts)s, %(prefix)sc_ktkb_%(sqrts)s, %(prefix)sc_ktkc_%(sqrts)s, %(prefix)sc_kbkc_%(sqrts)s, %(prefix)sc_kc2_%(sqrts)s, coeff_sum_%(scalingName)s\
            ...
            for qty, column in structure.items():
                ...
                self.textToSpline(
                    rooName,
                    os.path.join(self.coupPath, "Gamma_Hgluongluon.txt"),
                    ycol=column,
                )
...
"(@0*@0)*@2  + (@1*@1)*@3 + (@0*@1)*@4",\
 ...
 %(prefix)sGamma_tt, %(prefix)sGamma_bb, %(prefix)sGamma_tb\
            ...
            fileFor = {"hgg": "Gamma_Hgammagamma.txt", "hzg": "Gamma_HZgamma.txt"}
            structure = {
                "Gamma_tt": 2,
                "Gamma_bb": 3,
                "Gamma_WW": 4,
            ...
            for qty, column in structure.items():
                ...
                self.textToSpline(
                    ...
                    os.path.join(
                        self.coupPath,
                        fileFor["hgg" if what.startswith("hgg") else "hzg"],
                    ),
                    ycol=column,
                )
            scalingName = "Scaling_" + what
 ...
 %(prefix)sGamma_tl, %(prefix)sGamma_bl, %(prefix)sGamma_lW\
            ...
            for sqrts in ("7TeV", "8TeV", "13TeV", "14TeV"):
                for qty, column in structure.items():
                    ...
                    self.textToSpline(
                        rooName,
                        os.path.join(self.coupPath, "ggZH_%(sqrts)s.txt" % locals()),
                        ycol=column,
                    )
                scalingName = "Scaling_" + what + "_" + sqrts
                coeffSum = (
                    'expr::coeff_sum_%(scalingName)s( "@0+@1+@2+@3+@4+@5",%(prefix)sc_kt2_%(sqrts)s, %(prefix)sc_kb2_%(sqrts)s, %(prefix)sc_kZ2_%(sqrts)s, %(prefix)sc_ktkb_%(sqrts)s, %(prefix)sc_ktkZ_%(sqrts)s, %(prefix)sc_kbkZ_%(sqrts)s)'
                    % locals()
                )
                self.modelBuilder.factory_(coeffSum)

                rooExpr = (
                    'expr::%(scalingName)s( "( (@0*@0)*(@3)  + (@1*@1)*(@4) + (@2*@2)*(@5) + (@0*@1)*(@6)  + (@0*@2)*(@7) + (@1*@2)*(@8) )/@9",%(Ctop)s, %(Cb)s, %(CZ)s, %(prefix)sc_kt2_%(sqrts)s, %(prefix)sc_kb2_%(sqrts)s, %(prefix)sc_kZ2_%(sqrts)s, %(prefix)sc_ktkb_%(sqrts)s, %(prefix)sc_ktkZ_%(sqrts)s, %(prefix)sc_kbkZ_%(sqrts)s, coeff_sum_%(scalingName)s)'
                    % locals()
                )
                self.modelBuilder.factory_(rooExpr)
        elif what.startswith("tHq"):
            coeffs = {
                "7TeV": [3.099, 3.980, -6.078],
                "8TeV": [2.984, 3.886, -5.870],
                "13TeV": [2.633, 3.578, -5.211],
                "14TeV": [2.582, 3.538, -5.120],
            }  # coefficients for  kt^{2}, kW^{2}, ktkW @ MH = 125 GeV
            for sqrts in ("7TeV", "8TeV", "13TeV", "14TeV"):
                scalingName = "Scaling_" + what + "_" + sqrts
                rooExpr = (
            ...
            coeffs = {
                "7TeV": [2.306, 1.697, -3.003],
                "8TeV": [2.426, 1.818, -3.244],
                "13TeV": [2.909, 2.310, -4.220],
                "14TeV": [2.988, 2.397, -4.385],
            }  # coefficients for  kt^{2}, kW^{2}, ktkW @ MH = 125 GeV
            for sqrts in ("7TeV", "8TeV", "13TeV", "14TeV"):
                scalingName = "Scaling_" + what + "_" + sqrts
                rooExpr = (

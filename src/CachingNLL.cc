...
cacheutils::CachingAddNLL::evaluate() const 
{
    ...
    if (expectedEvents <= 0) {
    	...
    	Logger::instance().log(std::string(Form("CachingNLL.cc: %d -- underflow (expected events <=0) in total event yield for %s, expected yield = %g (observed: %g)",__LINE__,pdf_->GetName(), expectedEvents, sumWeights_)),Logger::kLogLevelInfo);
...
cacheutils::CachingSimNLL::evaluate() const 
{
    ...
    if (!constrainPdfs_.empty() || !constrainPdfsFast_.empty()) {
        ...
        for (std::vector<RooAbsPdf *>::const_iterator it = constrainPdfs_.begin(), ed = constrainPdfs_.end(); it != ed; ++it, ++itz) { 
    		...
    		Logger::instance().log(std::string(Form("CachingNLL.cc: %d -- underflow (pdf evaluates to <=0) of constraint pdf %s, value = %g ",__LINE__,(*it)->GetName(), pdfval)),Logger::kLogLevelInfo);

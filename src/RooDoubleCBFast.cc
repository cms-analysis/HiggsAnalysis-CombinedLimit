...
void RooDoubleCBFast::migrateFromV1(RooAbsReal& x,
                                    RooAbsReal& mean,
                                    RooAbsReal& width,
                                    RooAbsReal& alpha1,
                                    RooAbsReal& n1,
  ...
  auto setupUPtr = [&](const char* member, const char* pName, const char* pTitle, RooAbsReal& arg) {
    auto* uptr = reinterpret_cast<std::unique_ptr<RooRealProxy>*>(base + cbClass->GetDataMemberOffset(member));
...
#else  // ROOT < 6.38.02: original standalone implementation.

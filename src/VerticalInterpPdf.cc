...
Double_t VerticalInterpPdf::evaluate() const 
{
  ...
  Double_t value = (_quadraticAlgo >= 0) ? RooFit::Detail::MathFuncs::opInterpolate<std::plus<Double_t>>(
                                               _coefList, _funcList, _pdfFloorVal, _quadraticRegion, _quadraticAlgo)
                                         : RooFit::Detail::MathFuncs::opInterpolate<std::multiplies<Double_t>>(
                                               _coefList, _funcList, _pdfFloorVal, _quadraticRegion, _quadraticAlgo);
  return value;
}
...
Double_t VerticalInterpPdf::analyticalIntegralWN(Int_t code, const RooArgSet* normSet2, const char* /*rangeName*/) const 
{
  ...
  Double_t value = RooFit::Detail::MathFuncs::opInterpolate<std::plus<Double_t>>(
      _coefList, fIntL, _pdfFloorVal, _quadraticRegion, _quadraticAlgo, normSet2);

  Double_t normVal(1) ;
  RooArgList& fNormL = (dynamic_cast<CacheElem*>(_normIntMgr.getObjByIndex(0)))->_funcNormList;
  if (normSet2) {
    normVal = RooFit::Detail::MathFuncs::opInterpolate<std::plus<Double_t>>(
        _coefList, fNormL, _pdfFloorVal, _quadraticRegion, _quadraticAlgo, normSet2);
  }
...
{
  return RooFit::Detail::MathFuncs::interpolate(
      coeff, central, fUp->getVal(), fDn->getVal(), _quadraticRegion, _quadraticAlgo);
}

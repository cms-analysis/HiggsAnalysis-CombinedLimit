...
LimitAlgo("HybridNew specific options") {
    options_.add_options()
        ...
        ("fitNuisances", boost::program_options::value<bool>(&fitNuisances_)->default_value(fitNuisances_), "Fit the nuisance parameters first, before generating the toy data. Set this option to false to achieve the same results as with --bypassFrequentistFit. When not generating toys, eg as in when running with --readHybridresult, this has no effect")
        ...
        ("maxProbability", boost::program_options::value<float>(&maxProbability_)->default_value(maxProbability_),  "when point is >  maxProbability contour, don't bother throwing toys")

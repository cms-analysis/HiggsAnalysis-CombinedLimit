				 ...
				 Bool_t optConst, Bool_t verbose)
{
  ...
  for(index= 0; index < _constParamList->getSize() ; index++) {
    ...
    if (!par->isConstant()) {      
      ...
      if (!_floatParamList->find(*par))        _floatParamList->add(*par) ;
      if (!_initFloatParamList->find(*oldpar)) _initFloatParamList->addClone(*oldpar) ;      
  ...
  /*
  std::cout << " _constParamList --> " << _constParamList->getSize() << std::endl; 
  _constParamList->Print();
  std::cout << " _floatParamList --> " << _floatParamList->getSize() << std::endl; 
  _floatParamList->Print();
  std::cout << " _initConstParamList --> " << _initConstParamList->getSize() <<  std::endl; 
  _initConstParamList->Print();
  std::cout << " _initFloatParamList --> " << _initFloatParamList->getSize() <<std::endl; 
  _initFloatParamList->Print();
  */

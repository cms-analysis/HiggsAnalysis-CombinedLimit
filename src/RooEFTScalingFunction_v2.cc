#include "HiggsAnalysis/CombinedLimit/interface/RooEFTScalingFunction_v2.h"

ClassImp(RooEFTScalingFunction_v2)

RooEFTScalingFunction_v2::RooEFTScalingFunction_v2(const char *name, const char *title, const std::map<std::string,double> &coeffs, const RooArgList &terms) :
    ...
    offset_(1.0)
{
    ...
    for( RooAbsArg *a = iter.next(); a != 0; a = iter.next()) {
        ...
        if (!rar) {
            throw std::invalid_argument(std::string("Term ")+a->GetName()+" of RooEFTScalingFunction_v2 is a "+a->ClassName());
        }
        terms_.add(*rar);
    }
...
RooEFTScalingFunction_v2::RooEFTScalingFunction_v2(const RooEFTScalingFunction_v2& other, const char* name) :
...
Double_t RooEFTScalingFunction_v2::evaluate() const 
{

...
parser.add_option(
    ...
    action="store_true",
    default=False,
    help="Use correct XS*BR for Higgs (if not enabled, just makes a flat copy)",
)
...
parser.add_option(
    ...
    type="float",
    default="0",
    help="Reference mass to start from (default = nearest one, left in case of ties)",
)
parser.add_option(
    "--postfix",
    dest="postfix",
...
fileR = options.ddir + "/%g/vhbb_DC_ALL_%s.%.1f.txt" % (
    options.refmass,
    options.flavour,
    options.refmass,
)
options.fileName = fileR
options.mass = options.refmass
...
xfile = open(
    options.ddir + "/%g/vhbb_DC_ALL_%s%s.%.1f.txt" % (mass, options.flavour, options.postfix, mass),
    "w",
)

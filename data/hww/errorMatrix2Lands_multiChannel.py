...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Remove processes or systematics that don't contribute",
)
parser.add_option(
    "-g",
    "--gamma",
    ...
    default=False,
    action="store_true",
    help="Use gammas for statistical uncertainties",
)
parser.add_option(
    "--gmM",
    dest="gmM",
    ...
    action="append",
    default=[],
    help="Use multiplicative gamma for this systematics (can use multiple times)",
)
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Give names to systematics",
)
...
if options.asimov:
    for mh in data.keys():
        for i in range(options.nch):
            ...
            if options.signal:
                data[mh]["obs"][i] += data[mh]["exp"][start]
            if options.floor:
                data[mh]["obs"][i] = floor(data[mh]["obs"][i])
...
for mh, D in data.items():
    ...
    for inuis, N in enumerate(D["nuis"]):
        ...
        if options.gamma and isStat:
            ...
            name = "%-22s" % id
            # have to guess N
            sumN = sum([1.0 / (X * X) for X in N if X > 0])

...
parser.add_option(
    ...
    "--datacard",
    dest="datacard",
    help="datacard name [%default]",
    default="cms_datacard.txt",
)
parser.add_option(
    "-r",
    "--regularize",
    dest="reg",
    action="store_true",
    help="activate regularization [%default]",
    default=False,
)
...
for row in range(nbinsG):
    ...
    for col in range(nbinsR):
        S += mhat[(row, col)]
    for col in range(nbinsR):
        mhat[(row, col)] *= ROOT.TMath.Min(ROOT.TMath.Gaus(0.95, 0.02), 1.0) / S
...
for row in range(nbinsG):
    ...
    if not opts.reg or row == 0 or row == nbinsG - 1:
        ...
        constrains.append(
            "constr%d" % row + " constr r_Bin%(prev)d+r_Bin%(next)d-2*r_Bin%(bin)d %(reg)f" % {"bin": row, "next": row + 1, "prev": row - 1, "reg": delta}
        )  ## REG STRENGTH
...
if st == 0:
    print("Datacard: Success")
else:
    print("Datacard: Error")

...
class Model:
    def __init__(self):
        ...
        self.smearings = [0.75, 0.10, 0.025]  ## diagonal, off-diagonola, 2nd ...
        self.eff = [
            0.85,
            0.85,
            0.90,
    ...
    def info(self):
        ...
        print(
            "n. events: sig:",
            self.nevents,
            ("bkg: %s" % self.nbkg) if self.bkg else "bkg: None",
        )
    ...
    def doModel(self):
        ...
        self.Mhat = ROOT.TH2D(
            "Mhat",
            "response probability matrix (X=truth, Y=Reco)",
            self.nbins_x,
            xmin,
            xmax,
            self.nbins_y,
        ...
        for i in range(0, self.nbins_x):
            for j in range(0, self.nbins_y):
                ...
                if len(self.smearings) > abs(i - j):
                    self.Mhat.SetBinContent(binX, binY, self.smearings[abs(i - j)])
        ...
        if self.bkg:
            for i in range(0, self.nbins_y):
                ...
                self.x_cont.setRange(rangename, low, high)
                count = (
                    self.pdf_cont.createIntegral(
                        ROOT.RooArgSet(self.x_cont),
                        ROOT.RooArgSet(self.x_cont),
                        rangename,
                    ).getVal()
                    * self.nbkg
        ...
        self.M = ROOT.TH2D(
            "M",
            "response matrix (X=truth, Y=Reco)",
            self.nbins_x,
            xmin,
            xmax,
            self.nbins_y,
    ...
    def doWorkspace(self):
        ...
        self.MH = ROOT.RooRealVar("MH", "MH", 125.0)
        ...
        ## construct pdfs
        if self.type == "pdf":
            self.sigmaD = ROOT.RooRealVar("sigmaD", "sigma diagonal elements", 1.0)
            self.sigmaO = ROOT.RooRealVar("sigmaO", "sigma off-diagonal elements", 2.0)
        ...
        for i in range(0, self.nbins_x):
            for j in range(0, self.nbins_y):
                ...
                norm = ROOT.RooRealVar(
                    name + "_norm",
                    "normalization of xxx",
                    self.M.GetBinContent(binX, binY),
                )
                norm.setConstant()
                if self.type == "c&c":
    ...
    def doDatacard(self):
        ...
        for i in range(0, self.nbins_x):
            ...
            self.datacard.write(
                "## constr%d" % i
                + " constr r_Bin%(prev)d+r_Bin%(next)d-2*r_Bin%(bin)d delta[%(delta)s]\n" % {"bin": i, "next": i + 1, "prev": i - 1, "delta": 0.03}
            )
        ...
        for i in range(0, self.nbins_x):
            ...
            self.datacard.write(
                "## constr%d" % i
                + " constr (r_Bin%(prev)d-1.)*%(lambdaMC_prev)s+(r_Bin%(next)d-1.)*%(lambdaMC_next)s-2*(r_Bin%(bin)d-1.)*%(lambdaMC)s {r_Bin%(prev)d,r_Bin%(bin)d,r_Bin%(next)d,%(lambdaMC_vars)s} delta[%(delta)s]\n"
                % d
            )
        ...
        self.datacard.write(
            ...
            + expect
            + " -t -1 -m 125 --algo=grid --points=100 -P r_Bin1 --setParameterRanges r_Bin1=0.5,1.5 --floatOtherPOIs=1 "
            + rootname
            + "\n"
        )
        self.datacard.write("###############################\n")
